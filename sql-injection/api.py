...
def get_authors_dangerously_again(search_text):
    # Yikes! Here's where the injection happens. We're sticking
    # user-generated content (search_text) directly into our SQL query.
    # Don't do this!
    query = "SELECT given_name, surname FROM authors WHERE surname LIKE '%" + search_text + "%'"
    author_list = []
    try:
...
## THIS ROUTE IS ALSO BAD!! ##
@api.route('/authors/dangerous2/<search_text>')
